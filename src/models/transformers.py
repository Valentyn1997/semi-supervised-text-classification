...
class PretrainedTransformer(LightningModule):
    ...
    def train_dataloader(self) -> DataLoader:
        return DataLoader(self.train_dataset, shuffle=True, batch_size=self.args.data.batch_size.train, num_workers=10)

    def val_dataloader(self) -> DataLoader:
        return DataLoader(self.val_dataset, shuffle=False, batch_size=self.args.data.batch_size.val)

    def test_dataloader(self) -> DataLoader:
        return DataLoader(self.test_dataset, shuffle=False, batch_size=self.args.data.batch_size.test)
...
class SSLPretrainedTransformer(PretrainedTransformer):
    def __init__(self, args: DictConfig):
        ...
        self.weak_transform = naf.Sometimes([
            naw.WordEmbsAug(model_type='word2vec', model_path=f'{RESOURCES_PATH}/GoogleNews-vectors-negative300.bin',
                            action="substitute", aug_p=0.1),
            naw.SynonymAug(aug_src='wordnet', aug_p=0.1),
        ], pipeline_p=0.3)

        self.strong_transform = naf.Sometimes([
            # naw.BackTranslationAug(from_model_name='transformer.wmt19.en-de', to_model_name='transformer.wmt19.de-en'),
            naw.WordEmbsAug(model_type='word2vec', model_path=f'{RESOURCES_PATH}/GoogleNews-vectors-negative300.bin',
                            action="substitute", aug_p=0.1),
            naw.SynonymAug(aug_src='wordnet', aug_p=0.1),
            # nas.ContextualWordEmbsForSentenceAug(model_path='gpt2', device='cpu'),
            nas.AbstSummAug(model_path='t5-base', num_beam=3, max_length=0.8, device='cpu')
        ], pipeline_p=0.3)
    ...
    def training_step(self, composite_batch, batch_idx):
        l_batch, ul_batch = tuple([torch.cat(sub_item) for sub_item in zip(*item)] for item in composite_batch)

        # print(ul_batch[0].shape, l_batch[0].shape)
    ...
    def load_and_cache_examples(self, mode):
        if mode == 'val' or mode == 'test':
            ...
            train_l = AugmentableTextClassificationDataset(examples_l, weak_transform=self.weak_transform,
                                                           ...
                                                           max_seq_length=self.args.data.max_seq_length,
                                                           model_type=self.args.model.model_type)
            train_ul = AugmentableTextClassificationDataset(examples_ul,
            ...
            return FixMatchCompositeTrainDataset(train_l, train_ul, self.args.ssl.mu)
